...
object ApiRole1 extends App{
  ...
  //  println("Hongwei !!!!") //TODO, you can optimise these variables.

  private val roles: List[ApiRole with Product with Serializable] =
  ...
  val allFields =
    for (
      v <- this.getClass.getDeclaredFields if (v.isInstanceOf[ApiRole])
    )
      yield {
